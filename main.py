...
async def gen(ctx, gif: str, pfp_id: int):
    # try:
    if gif.lower() in gifs:
        if 0 < pfp_id <= 15000:
...
async def wallpaper(ctx, wallpaper: str, pfp_id: int):
    ...
    if wallpaper.lower() in phone_backgrounds:
        if 0 < pfp_id <= 5001:
            make_wallpaper(wallpaper.lower().lower(), str(pfp_id))
            await ctx.send(file=discord.File(save_img_folder + wallpaper.lower() + str(pfp_id) + '.png'))
            deleteDressed(wallpaper.lower().lower(), str(pfp_id))
...
@bot.command(name="no_monke_banner", brief='Phone Wallpaper',
             description='This command will let make a phone wallpapere, type `?wallpapers` to see available backgrounds')
async def no_background_banner(ctx, wallpaper: str):
    # try:
